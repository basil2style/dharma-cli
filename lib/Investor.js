...
import { addInvestment, initState, log, updateTotalCash, updatePortfolioSummary } from './actions/actions';
...
class Investor {
  ...
  async startDaemon(store, errorCallback) {
    ...
    this.createdEvent.watch(async function(err, result) {
      ...
      if (bid) {
        ...
        investment.state = AUCTION_STATE;
  ...
  async refreshInvestment(investment) {
    const state = investment.state;
  ...
  auctionCompletedCallback(uuid, auctionCompletedEvent) {
    ...
    return async (err) => {
      auctionCompletedEvent.stopWatching(async () => {
        _this.portfolio.getInvestment(uuid).state = REVIEW_STATE;
  ...
  termBeginCallback(uuid, termBeginEvent) {
    ...
    return async (err, result) => {
      ...
      termBeginEvent.stopWatching(async () => {
        ...
        investment.termBeginDate = new Date();
        investment.state = ACCEPTED_STATE;
        investment.repaymentStatus = REPAYMENT_STATUS.CURRENT;

        this.setupRepaymentDateJobs(investment);

        await _this.refreshInvestment(investment);

        const portfolioSummary = await _this.portfolio.getSummary();

        _this.store.dispatch(addInvestment(investment));
  ...
  bidsRejectedCallback(uuid, bidsRejectedEvent) {
    ...
    return (err) => {
      ...
      bidsRejectedEvent.stopWatching(async () => {
        ...
        investment.withdrawn = true
        investment.state = REJECTED_STATE;
        await _this.savePortfolio();
      })
  ...
  repaymentCallback(uuid, repaymentEvent) {
    const _this = this;
