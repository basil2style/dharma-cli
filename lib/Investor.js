...
class Investor {
  ...
  async startDaemon(store) {
    ...
    process.on('uncaughtException', function(err) {
      this.store.dispatch(log('error', err.toString()));
    })
  ...
  async redeemValueIfPossible(uuid) {
    const investment = this.portfolio.investments[uuid];
    const loan = investment.loan;

    const redeemableValue =
      await loan.getRedeemableValue(this.wallet.getAddress());
  ...
  async setupAcceptedState(bid) {
    ...
    if (tokenBalance.equals(0))
      this.store.dispatch(log('info', 'Lost auction for loan ' + loan.uuid));
  ...
  repaymentCallback(uuid) {
    const callback = async (err, result) => {
      ...
      this.store.dispatch(log('info', 'Collecting repayment from loan contract ' +
        uuid));
      await this.redeemValueIfPossible(uuid);
    }
  ...
  async loadPortfolio() {
    try {
      this.portfolio = await Portfolio.load(this.dharma, this.wallet);
    } catch (err) {
      this.portfolio = new Portfolio(this.dharma.web3, this.wallet);
    }
  ...
  async collect(uuid) {
    const investment = this.portfolio.investments[uuid];
    this.store.dispatch(log('info', 'Redeeming repaid value from loan ' + uuid))
    try {
